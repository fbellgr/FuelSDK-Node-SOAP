 ...
 * @param {String} action - Perform action as `PerformRequestMsg.Action`
 * @param {Object} options - PerformOptions as `PerformRequestMsg.Options`
 * @param {FuelSoap~StandardCallback} callback - Callback that handles response
 */
 FuelSoap.prototype.perform = function(type, def, action, options, callback) {
...
FuelSoap.prototype._parseResponse = function(key, body, callback) {
	var parseOptions = {
		trim: true
		// , normalize: true // this breaks data extension records that contains new lines
		, explicitArray: false
		, ignoreAttrs: true
	};
 ...
 * @memberof FuelSoap
 * @param {Object} def - Set in body as `ExtractRequestMsg.Requests`
 * @param {FuelSoap~StandardCallback} callback - Callback that handles response
 */
